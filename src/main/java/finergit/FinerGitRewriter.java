...
import finergit.ast.FinerFileBuilder;
...
public class FinerGitRewriter extends RepositoryRewriter {
  ...
  private final FinerFileBuilder builder;

  public FinerGitRewriter(final FinerGitConfig config) {
    this.config = config;
    this.builder = new FinerFileBuilder(config);
  ...
  public EntrySet rewriteEntry(final Entry entry, final Context c) {
    ...
    if (entry.name.endsWith(".java")) {
      for (final FinerModule m : extractFinerModules(entry, c)) {
      ...
      List<FinerModule> finerModules = extractFinerKotlinModules(entry, c);
      for (final FinerModule m : finerModules) {
        final String finerSource = // 最終行に改行を入れないと途中行とのマッチングが正しく行われない
                String.join(System.lineSeparator(), m.getLines()) + System.lineSeparator();
        final ObjectId newId = target.writeBlob(finerSource.getBytes(StandardCharsets.UTF_8), c);
        final String name = m.getFileName();
      ...
      throw new IllegalStateException("Unexpected file type: " + entry.name);
    }
    return result;
  }

  protected List<FinerModule> extractFinerModules(final Entry entry, final Context c) {
  ...
  protected List<FinerModule> extractFinerKotlinModules(final Entry entry, final Context c) {
    final String base = entry.directory + "/" + entry.name;
    final String body = new String(source.readBlob(entry.id, c), StandardCharsets.UTF_8);
    return builder.getFinerKotlinModules(base, body);
  }
}
